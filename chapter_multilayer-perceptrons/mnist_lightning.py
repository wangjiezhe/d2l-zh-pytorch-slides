...
class MNISTModel(LightningModule):
  def __init__(self, batch_size=256, num_hiddens=1024, lr=0.5, data_dir="../data", num_workers=8):
    ...
    self.net = nn.Sequential(nn.Flatten(), nn.Linear(28*28, self.num_hiddens * 2), nn.Dropout(),
                             nn.ReLU(), nn.Linear(self.num_hiddens * 2, 10))
  ...
  def setup(self, stage=None):
    self.train_data, self.val_data = random_split(
      MNIST(root=self.data_dir, train=True, transform=self.trans),
      [0.8, 0.2])
